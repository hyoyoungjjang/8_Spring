 ...
 * - Component : Spring이 해당 빈을 서칭할 수 있도록 선언함(빈에 등록)
...
public class LoggingAOP {
	...
	/**
	 * <시점>
	 * @Before : 대산메서드 실행 전에 Advice(추가기능)이 실행된다.
	 * 
	 *  @After : 대상 메서드 실행 후에 Advice(추가기능)이 실행된다.
	 *  
	 *  @AfterReturning : 대상 메서드가 정상적으로 반환된 후에 Advice가 실행
	 *  
	 ...
	 * within : 특정 패키지또는 클래스를 지칭
	 * execution : 표현식으로 형채지정
	 */
	

	// @Pointcut - 내가 기능을 사용할 지점을 정의
	// com.kh.spring패키지 하위 패키지중 controller내에 있는 모든 클래스의 모든메서드
