...
//	private MemberService memberService = new MemberServiceImpl();
	 ...
	 * 해당메소드의 매개변수로 HttpServletRequest를 작성해두면
...
//	@RequestMapping("login.me")
//	public String loginMember(@RequestParam String userId, @RequestParam String userPwd) {
//		
//		System.out.println("userId : " + userId);
//		System.out.println("userPwd : " + userPwd);
//		return "main";
...
//	}
	 ...
	 * 요청시 전달값의 키값(jsp의 naem속성값)을 vo클래스에 담고자하는 필드명으로 작성하면 된다.
	 */
...
//		
//		Member loginUser = memberService.lgoinMember(m);
//		
//		if(loginUser == null) {
...
//	}
	 ...
	 * request.setAttribute() -> model.addAttribute9)
	 */
...
//			
//			// /WEB-INF/views/common/arrorPage.jsp
...
//		}
		...
		//loginUser id => 아이디로 db에서 검사해온 id
		//loginUser pwd => db에 기록되 암호화 비밀번호
		
		//bcryptPasswordEncoder객체의 matches()
		//matches(평문, 암호문)을 작성하면 내부적으로 복구화작업 비교가 이루어짐
		// 두구문이 일치하면 true를 반환 일치하지않으면 false반환
		...
		if(loginUser == null) { //아이디가 없는 경우
			...
			mv.setViewName("common/errorPage");
		} else if (!bcryptPasswordEncoder.matches(m.getUserPwd(), loginUser.getUserPwd())) {//비밀번호가 다른경우
	...
	public ModelAndView enrollFormMember(ModelAndView mv) {
	...
	/*
	 * ajax요청에 대한 응답을 위한 cpntroller에는 @ResponseBody어노케이션을 작성해줘야한다
	 * 기본적인 세팅이 jsp응답으로 되어있기 때무네 @ ResponseBody을 작성해주면
	 * 반환값 http응답 ㄱ개체에 직접 작성하겠다는 의미를 가지고있다.
	 */
	//idChceck ajax요청을 받아줄 Controller
	@ResponseBody
	@RequestMapping("idCheck.me")
	public String idChck(String checkId) {
	...
	public String deleteMember(Member m, HttpSession session ) {
		...
		if(bcryptPasswordEncoder.matches(m.getUserPwd(), encPwd)) { // 
			...
			if(result > 0) {
				...
				session.setAttribute("alerMsg", "탈퇴처리 실패");
				return "redirect:/mypage.me";
			}
		} else {
