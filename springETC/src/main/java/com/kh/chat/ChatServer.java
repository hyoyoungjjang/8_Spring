...
public class ChatServer extends TextWebSocketHandler{
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		//다른 사용자에게 메세지 전송
		sendMessageUsre(obj.get("target").getAsString(), vo);
		
	}
		
	//특정 사용자에게 메세지를 전송하는 메서드
	private void sendMessageUsre(String targetNick, MsgVo msgVo) {
		//target의 세션을 세션목록으로부터 가져옴
		WebSocketSession targetSession = userSessions.get(targetNick);	
		
		//현제 사용자의 세션을 세션목록으로부터 가져옴
